...
export const FilterInputField = ({ name, type }: FilterFieldProps) => {
  const { retrieveFilterByKmPriceAdvert, minKm, minPrice, maxKm, maxPrice } = useAdverts();
    ...
    <div className="mb-5">
      ...
      <div className="flex  pl-2.5 mt-2.5 gap-2">
        <input
          ...
          type="number"
          placeholder="Mínimo"
          value={minKm != 0 && type == "KM" ? minKm : minPrice != 0  && type == "Price" ? minPrice : ''}
        />
        <input
          className="input-base
          input-placeholder
          ...
          type="number"
          placeholder="Máximo"
          value={maxKm != 1000000 && type == "KM" ? maxKm : maxPrice != 1000000  && type == "Price" ? maxPrice : ''}
        />
      </div>
    </div>
  );
