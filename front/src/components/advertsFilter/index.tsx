...
export default function AdvertsFilter({list, hidden, setHidden}: AdvertsFilterProps) {

    const { retrieveAdvert, page, setMinKm, setMaxKm, setMinPrice, setMaxPrice } = useAdverts();

    return (
        <section className={`flex flex-col gap-10 lg:w-[21%] py-16 px-4 ${hidden && "hidden"} lg:flex`}>
            <div>
            ...
            <div className="w-full self-center lg:hidden"><Button type="outlineBrand1" handle={() => setHidden(true)}>Ver anúncios</Button></div>
            {(page?.filter || page?.filterMax || page?.filterMin)  && <Button type="brand" handle={() => {retrieveAdvert("", "", 1); setMinKm(0); setMaxKm(1000000); setMinPrice(0); setMaxPrice(1000000)}}>Limpar Filtros</Button>}
        </section>
